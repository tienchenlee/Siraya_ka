...
if __name__ == "__main__":
    ...
    #for valueLIST in globalDICT.values():
        #for valueSTR in valueLIST:
            #match = re.search(r"(?:[a-z’']+[\.\-]){1,5}[a-z’']+", valueSTR)
            #if match and match.group() not in glossLIST:
                #glossLIST.append(match.group())

    #print(glossLIST)

    udPATH = dataDIR / "userDefined.json"
    with open(udPATH, "r", encoding="utf-8") as f:
        udDICT = json.load(f)

    allValueLIST = []
    for v_l in udDICT.values():
        for v_s in v_l:
            if v_s not in allValueLIST:
                allValueLIST.append(v_s)

    #for g in glossLIST:
        #if g not in allValueLIST:
            #print(g)

    udEvalPATH = dataDIR / "ud_eval.json"
    with open(udEvalPATH, "r", encoding="utf-8") as f:
        udEvalLIST = json.load(f)

    toAddLIST = []
    for i in udEvalLIST:
        if i not in allValueLIST:
            toAddLIST.append(i)
