...
"""
    Loki module for CP_V2_and_VP_and_VP
...
INTENT_NAME = "CP_V2_and_VP_and_VP"
...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "believe .AV -IRR me -OBL ka be.at .AV I .NOM OBL father ka be.at .AV also NOM father me -OBL ka if not then believe .AV -IRR me -OBL because OBL work itself .AV":
        if CHATBOT:
    ...
    if utterance == "see -LV GEN Herod ka PAST- deceive .AV him -OBL NOM wise observe then exceeding .AV angry .AV NOM he ka when PAST- send.forth .AV PAST- kill .AV OBL all OBL boys ka LOC Bethlehem LOC all -also OBL borders its ka from.two .AV OBL year ka under .AV also":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("believe .AV -IRR me -OBL ka be.at .AV I .NOM OBL father ka be.at .AV also NOM father me -OBL ka if not then believe .AV -IRR me -OBL because OBL work itself .AV", "believe .AV -IRR me -OBL ka be.at .AV I .NOM OBL father ka be.at .AV also NOM father me -OBL ka if not then believe .AV -IRR me -OBL because OBL work itself .AV", [], {}, {})
    pprint(resultDICT)