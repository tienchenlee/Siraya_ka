...
MODULE_DICT = {
    "Account": import_from_path("Siraya_ka_lib_Account", os.path.join(os.path.dirname(CWD_PATH), "lib/Account.py")),
    "LLM": import_from_path("Siraya_ka_lib_LLM", os.path.join(os.path.dirname(CWD_PATH), "lib/LLM.py"))
}
"""
Account Variables
...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "PAST- hear -PV you .PL .GEN ka":
        if CHATBOT:
            ...
            resultDICT[INTENT_NAME].append(True)


    if utterance == "PAST- see -LV we .EXCL .GEN you .SG -OBL ka hungry .AV ka thirsty .AV ka be.stranger .AV":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
    ...
    if utterance == "good .AV -IRR you .SG -OBL ka":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
    ...
    if utterance == "this NOM will OBL father ka PAST- send.forth .AV me -OBL ka":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("so .AV .IRR .PFV ka", "so .AV .IRR .PFV ka", [], {}, {})
    pprint(resultDICT)