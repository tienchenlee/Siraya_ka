...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "leave .AV -IRR NOM human OBL father OBL mother ka join .AV -IRR OBL wife his ka two ka those one -IRR OBL flesh":
        if CHATBOT:
    ...
    if utterance == "sit .AV -IRR -PFV NOM son OBL man LOC seat OBL far.above OBL shine -PV OBL status his ka sit .AV -IRR also you .PL .NOM LOC one ten plus OBL two OBL seat ka far.above":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
    ...
    if utterance == "take -PV .IRR NOM child with.together -PV .IRR DET mother his ka travel .AV -IRR go .AV LOC land OBL Israel for PAST- die .AV NOM they ka PAST- seek .PV they .GEN NOM soul OBL child":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("leave .AV -IRR NOM human OBL father OBL mother ka join .AV -IRR OBL wife his ka two ka those one -IRR OBL flesh", "leave .AV -IRR NOM human OBL father OBL mother ka join .AV -IRR OBL wife his ka two ka those one -IRR OBL flesh", [], {}, {})
    pprint(resultDICT)