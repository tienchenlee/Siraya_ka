...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "when ka take.together .AV him -OBL DET Peter -OBL OBL two also ka DET sons GEN Zebedee PAST- PC. begin .AV sad .AV PC. great .AV also heavy .AV NOM he":
        if CHATBOT:
    ...
    if utterance == "not for.long .AV ka not see .AV -IRR me -OBL NOM world":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
