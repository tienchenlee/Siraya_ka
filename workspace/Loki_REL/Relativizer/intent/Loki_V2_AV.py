...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "NOM cause.live .AV OBL man rather LOC all-instead OBL word ka originate .AV appear .AV LOC mouth OBL God":
        if CHATBOT:
    ...
    if utterance == "PAST- hear .AV NOM DET Herod ka one OBL man OBL four OBL ruler OBL land OBL rumor GEN Jesus":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
    ...
    if utterance == "accept .AV OBL word this only them -OBL ka PAST- give -LV -PFV .":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
    ...
    if utterance == "swear do.not PC. heaven .AV because ka greatness that ka seat OBL God":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
    ...
    if utterance == "understand .AV NOM anyone not also NOM angel ka LOC far.above OBL heaven":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("teach .AV OBL doctrine ka PART commandment OBL man", "teach .AV OBL doctrine ka PART commandment OBL man", [], {}, {})
    pprint(resultDICT)