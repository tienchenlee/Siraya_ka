...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "PAST- bring .AV him -OBL OBL many .AV ka PAST- possessed.by.devil ka PAST- drive.out -PV he .GEN OBL word NOM spirit ka evil .AV ka PAST- make.well .AV all them -OBL ka not good .AV OBL body":
        if CHATBOT:
    ...
    if utterance == "eat .AV ka be.satisfied .AV -PFV ka PAST- take .PV they .GEN collect OBL PAST- PC. left.over .AV OBL PART fragments seven ka basket -LV ka full .AV however":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
    ...
    if utterance == "cast.hook -IV .IRR NOM hook ka take -PV .IRR NOM fish ka PC. first .AV come.up .AV ka when open -PV .IRR NOM mouth its find -PV .IRR you .SG .GEN NOM Stater ka silver":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("eat .AV ka be.satisfied .AV -PFV ka PAST- take .PV they .GEN collect OBL PAST- PC. left.over .AV OBL PART fragments seven ka basket -LV ka full .AV however", "eat .AV ka be.satisfied .AV -PFV ka PAST- take .PV they .GEN collect OBL PAST- PC. left.over .AV OBL PART fragments seven ka basket -LV ka full .AV however", [], {}, {})
    pprint(resultDICT)