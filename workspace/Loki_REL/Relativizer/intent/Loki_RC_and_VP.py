...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "not yet you ,PL .GEN understand -LV whatever ka go.into.mouth .AV ka go.into.belly .AV ka throw .PV LOC draught NOM it":
        if CHATBOT:
    ...
    if utterance == "whatever ka from-mouth .AV go.out .AV from-heart .AV ka that instead NOM cause.rotten .AV contaminate .AV OBL man":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
    ...
    if utterance == "all ka PAST- hear .AV OBL this OBL father ka PAST- teach -PV also NOM come .AV me -OBL":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("whatever ka from-mouth .AV go.out .AV from-heart .AV ka that instead NOM cause.rotten .AV contaminate .AV OBL man", "whatever ka from-mouth .AV go.out .AV from-heart .AV ka that instead NOM cause.rotten .AV contaminate .AV OBL man", [], {}, {})
    pprint(resultDICT)