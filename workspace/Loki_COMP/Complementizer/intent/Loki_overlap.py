...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "witness you .PL .NOM me -OBL NOM you .PL self .AV ka PAST- speak -PV I .GEN not DET I NOM DET Christ":
        if CHATBOT:
    ...
    if utterance == "when ka give.mercy .AV you .SG .NOM OBL mercy":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
    ...
    if utterance == "this -instead NOM judgment ka come .AV -PFV NOM light LOC world":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
